...
SELECT
    ...
    e.first_name || ' ' || e.last_name AS full_name,
    e.department,
    ROUND(COALESCE(AVG(a.hours_worked), 0)::numeric, 2) AS avg_hours_worked,
...
dept_top_employee AS (
    SELECT
        e.department,
        e.employee_id,
        e.first_name || ' ' || e.last_name AS top_employee_name,
        e.salary
    FROM employees e
    JOIN (
        ...
        FROM employees
        GROUP BY department
    ) mx ON e.department = mx.department AND e.salary = mx.max_salary
)
SELECT
    ds.department,
    ds.total_employees,
    ds.avg_salary,
    ds.max_salary AS highest_salary,
    COALESCE(string_agg(DISTINCT dte.top_employee_name, ', '), '') AS highest_paid_employee_names,
    GREATEST(ds.max_salary, ds.avg_salary) AS greatest_of_max_and_avg,
    LEAST(ds.max_salary, ds.avg_salary) AS least_of_max_and_avg
FROM dept_stats ds
LEFT JOIN dept_top_employee dte ON ds.department = dte.department
GROUP BY ds.department, ds.total_employees, ds.avg_salary, ds.max_salary
